...
class App extends Component {
  ...
  getRandomItem = () => {
    ...
    this.setState({currentImage: imagesList[randomIndex]})
  }
  ...
  handleClick = id => {
    ...
    if (id === currentImage.id) {
      this.setState(prevState => ({
        currentImage: this.getRandomItem(),
  ...
  clickPlay = () => {
    this.setState({gameEnd: false, score: 0, time: 60}, this.updateTimer)
  }

  render() {
    const {activeTabId, currentImage, time, score, gameEnd} = this.state
      ...
      <div className="app-container">
        ...
        {gameEnd ? (
          <div className="score-card-container">
            <ScoreCard score={score} clickPlay={this.clickPlay} />
          </div>
        ) : (
